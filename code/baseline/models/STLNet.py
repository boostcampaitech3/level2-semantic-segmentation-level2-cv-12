...
'''
reference code by https://github.com/lanyunzhu99/Learning-Statistical-Texture-for-Semantic-Segmentation
'''
...
class deconvBNReLU(nn.Module):
    '''Module for the Transposed Conv-BN-ReLU tuple.'''
...
class PTFEM(nn.Module):
    def __init__(self):
        ...
        self.qco_4 = QCO_2d(4, 8)
        self.qco_8 = QCO_2d(8, 8)
        self.out = ConvBNReLU(128*4, 256, 1, 1, 0)
    def forward(self, x):
        H, W = x.shape[2:]
        x = self.conv(x)
        sta_1 = self.qco_1(x) 
        ...
        sta_4 = sta_4.view(N, C, 4, 4)
        sta_8 = sta_8.view(N, C, 8, 8)
        ...
        sta_4 = F.interpolate(sta_4, size=(H, W), mode='bilinear', align_corners=True)
        sta_8 = F.interpolate(sta_8, size=(H, W), mode='bilinear', align_corners=True)
        x = torch.cat([sta_1, sta_2, sta_4, sta_8], dim=1) 
        # x = torch.cat([sta_1, sta_2], dim=1) 
        x = self.out(x)
        return x
...
class STL(nn.Module):
    def __init__(self):
        ...
        #backbone #오류발생시 `pip install -U torchvision`
        self.deeplabv3 = torch.hub.load('pytorch/vision:v0.10.0', 'deeplabv3_resnet101', pretrained=True)
        self.backbone = nn.Sequential(OrderedDict([layer for layer in self.deeplabv3.backbone.named_children()]))
        self.aspp = self.deeplabv3.classifier[0]
        
        #layer2
        self.layer1 = self.backbone[:5]
        self.layer1_scaling = ConvBNReLU(256, 256, 3, 2, 1)
        
        #layer2
        self.layer2 = self.backbone[5:6]
        
        #layer3, 4
        self.layer34 = self.backbone[6:8]
        self.reduce_channel = ConvBNReLU(256+512, 256, 1, 1, 0)
        ...
        #upsample
        self.deconv1 = deconvBNReLU(1024, 256, kernel_size=4, stride=2, padding=1)
        self.deconv2 = deconvBNReLU(256, 128, kernel_size=4, stride=2, padding=1)
        self.deconv3 = deconvBNReLU(128, 11, kernel_size=4, stride=2, padding=1)

    def forward(self, x):
        x1 = self.layer1(x)
        # print('x1-->', x1.shape)
        x1_scaled = self.layer1_scaling(x1)
        ...
        x12_concat = torch.cat([x1_scaled, x2], dim=1)
        x12_concat = self.reduce_channel(x12_concat)
        # print('x12_concat-->', x12_concat.shape)

        
        x_tem = self.tem(x12_concat)
        # print('x_tem --> ', x_tem.shape)
        x = torch.cat([x_tem, x12_concat], dim=1) #c = 256 + 256 = 512
        # print('x_tem+x12_concat --> ', x.shape)
        x_ptfem = self.ptfem(x) # 256   
        # print('x_ptfem --> ', x_ptfem.shape)
        x = torch.cat([x_ptfem, x, aspp], dim=1)
        # print('x_ptfem+x+aspp --> ', x.shape)
